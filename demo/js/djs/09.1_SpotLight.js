...
const FSHADER_SOURCE =
    ...
    vec3 lightWeighting=vec3(.0,.0,.0);   

    void main(){
        //计算出方向指向光源的向量
        vec3 vectorToLightSource=normalize(u_LightPosition-vPositionEye3);
		//通过点积算出光线的亮度权重
		float diffuseLightWeighting=max(0.0,dot(vNormalEye,vectorToLightSource));
        ...
        if (diffuseLightWeighting>0.0)
        {
            ...
            if (spotEffect>spotCosCutoff)
            {
                ...
                //眼睛坐标下的相机位于原点，并沿着Z轴指向，计算眼坐标中的视向量
                ...
                lightWeighting=spotEffect*u_DiffuseLightColor*diffuseLightWeighting+spotEffect*uSpecularLightColor*specularLightWeighting;
            }
        }
        //加上环境光
        lightWeighting+=u_AmbientLightColor;
...
function setupLights() {
    gl.uniform3fv(pwgl.uniformLightPositionLoc, [.0, 10.0, -12.0]);
